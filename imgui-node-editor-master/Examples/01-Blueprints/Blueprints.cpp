...
#include "Builders.h"
#include "Widgets.h"
#include "..\..\..\Imgui Node\Examples\01-Blueprints\CFileReader.h"
...
static Node* SpawnInitNode()
{
	s_Nodes.emplace_back(GetNextId(), "Init");
	s_Nodes.back().Type = NodeType::Blueprint;
	s_Nodes.back().Outputs.emplace_back(GetNextId(), "Start", PinType::Flow);
...
static 	std::vector<SNodeData> node_data;


static Node* BuildNodeFromNodeData(SNodeData& aNodeData)
{
	s_Nodes.emplace_back(GetNextId(), aNodeData.name.c_str());
	
	for (SPinData& out_pin : aNodeData.outs)
	{
		PinType pintype;

		if (out_pin.type == "Flow")
		{
			pintype = PinType::Flow;
		}
		else if (out_pin.type == "Bool")
		{
			pintype = PinType::Bool;
		}
		else if (out_pin.type == "Int")
		{
			pintype = PinType::Int;
		}
		else if (out_pin.type == "Float")
		{
			pintype = PinType::Float;
		}
		else if (out_pin.type == "Object")
		{
			pintype = PinType::Object;
		}
		else if (out_pin.type == "Function")
		{
			pintype = PinType::Function;
		}
		else if (out_pin.type == "Delegate")
		{
			pintype = PinType::Delegate;
		}
		else if (out_pin.type == "String")
		{
			pintype = PinType::String;
		}


		s_Nodes.back().Outputs.emplace_back(GetNextId(), out_pin.name.c_str(), pintype);
	}
	for (SPinData& in_pin : aNodeData.ins)
	{
		PinType pintype;
		...
		else if (in_pin.type == "Bool")
		{
			pintype = PinType::Bool;
		}
		else if (in_pin.type == "Int")
		{
			pintype = PinType::Int;
		}
		else if (in_pin.type == "Float")
		{
			pintype = PinType::Float;
		}
		else if (in_pin.type == "Object")
		{
			pintype = PinType::Object;
		}
		else if (in_pin.type == "Function")
		{
			pintype = PinType::Function;
		}
		else if (in_pin.type == "Delegate")
		{
			pintype = PinType::Delegate;
		}
		else if (in_pin.type == "String")
		{
			pintype = PinType::String;
		}

		s_Nodes.back().Inputs.emplace_back(GetNextId(), in_pin.name.c_str(), pintype);
	}
...
void Application_Initialize()
{
	...
	CFileReader reader;
	std::vector<std::string> nodePaths = reader.ReadDirectory("Nodes");



	for (std::string& path : nodePaths)
	{
		SNodeData n_data;

		READ_ERROR_CODE result = reader.ReadNode(path, n_data);
		if (result != READ_ERROR_CODE::Success)
		{
			assert(false);
		}
		node_data.push_back(n_data);
	}
...
ImColor GetIconColor(PinType type)
{
    switch (type)
    {
		...
		case PinType::String:   return ImColor(255, 100, 100);
    }
};
...
void Application_Frame()
{
    ...
    if (ImGui::BeginPopup("Create New Node"))
    {
        ...
        Node* node = nullptr;
		//if (ImGui::MenuItem("Init Node"))
		//{
		//	node = SpawnInitNode();
		//}

		for (SNodeData& n_data : node_data)
		{
			if (ImGui::MenuItem(n_data.name.c_str()))
			{
				node = BuildNodeFromNodeData(n_data);
			}
		}
